...
def build_interface():
    with gr.Blocks(theme=gr.themes.Default(primary_hue="blue")) as app:
        ...
        gr.Markdown(
            ...
            "Look into the camera. When a face is detected, a capture will be made after 3 seconds."
        )

        with gr.Row():
            webcam_input = gr.Image(sources=["webcam"], type="numpy", label="Live Webcam")
            before_display = gr.Image(type="numpy", label="Original (Captured)")
            simulated_display = gr.Image(type="numpy", label="Simulated Output")
        
        print_button = gr.Button("Print Results", visible=False)

        def process_webcam_stream(frame, state):
            button_update = gr.update(visible=False)

            if frame is None:
                return state['before_image'], state['output_image'], state, button_update

            has_face = landmarks_from_image_rgb(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)) is not None
            
            if has_face:
                if state['detection_time'] is None:
                    state['detection_time'] = time.time()
                
                elif state['detection_time'] != float('inf') and (time.time() - state['detection_time'] > 3):
                    simulated = simulate(frame, side="right", severity=0.62, lateral=0.06)
                    state['before_image'] = frame
                    state['output_image'] = simulated
                    state['detection_time'] = float('inf')
            else:
                state['detection_time'] = None
                state['before_image'] = None
                state['output_image'] = None

            if state['detection_time'] == float('inf'):
        ...
        webcam_input.stream(
            ...
            inputs=[webcam_input, app_state],
            outputs=[before_display, simulated_display, app_state, print_button]
        )

        print_button.click(fn=None, _js="() => { window.print(); }")
